...
export function Modal({ isOpen, onClose, title, children, maxWidthClass = "max-w-lg" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 sm:p-4 print:static print:block print:bg-transparent print:p-0">
      <div
        className={`flex h-full w-full flex-col bg-white/95 dark:bg-slate-900/95 sm:h-auto sm:max-h-[90vh] sm:w-full sm:${maxWidthClass} sm:rounded-2xl shadow-2xl print:h-auto print:w-auto print:max-h-none print:rounded-none print:shadow-none print:bg-white`}
      >
        {title ? (
